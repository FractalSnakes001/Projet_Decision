...
def NombrePreferance(profilA,K,L,M=None):
    """
    ...
    for bull in profilA:
        ...
        #aussi pour le type L, si K est mieux que L alors Bk > Bl les deux dans {1..M}
        if bull[K] > bull[L]: 
            NbPref+=1
    return NbPref
...
def get_PolarDistances(profil):
    ...
    for k in range(0,M-1):
        for l in range(k+1,M):
            Couple = (k,l)
            Pk_L = NombrePreferance(profil,k,l,M)
            Pl_K = NombrePreferance(profil,l,k,M)
